...
MainMenu::MainMenu()
{
    ...
    this->_socTempLabel = new tsl::elm::ListItem("SOC 温度: --℃");
    ...
    if (IsRunning() != 0)
    {
        this->_enabledBtn = new tsl::elm::ToggleListItem("应用", true);
    }
    else
    {
        this->_enabledBtn = new tsl::elm::ToggleListItem("应用", false);
    }
}
...
void MainMenu::update()
{
    ...
    if (counter % 6 == 0) {
        ...
        if (socTemp >= 0) {
            this->_socTempLabel->setText("SOC 温度: " + std::to_string((int)socTemp) + "℃");
        } else {
            this->_socTempLabel->setText("SOC 温度: 错误");
        }
